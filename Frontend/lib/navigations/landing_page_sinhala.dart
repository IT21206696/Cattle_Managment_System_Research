...
import 'package:chat_app/navigations/home_screen_sinhala.dart';
...
import 'package:chat_app/navigations/signup_window_sinhala.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:provider/provider.dart';
import 'package:shared_preferences/shared_preferences.dart';

class LandingPageSinhala extends StatefulWidget {
  const LandingPageSinhala({Key? key}) : super(key: key);

  @override
  _LandingPageSinhalaState createState() => _LandingPageSinhalaState();
}

class _LandingPageSinhalaState extends State<LandingPageSinhala> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('ප්‍රවේශය සාර්ථකයි!')),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('නොයෙදුම් නාමය හෝ මුරපදය වැරදියි')),
        );
      }
    } catch (error) {
      print(error);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('දෝෂයක් ඇතිවිය: $error')),
      );
    }
              ...
              child: Column(
                children: [
                  TextField(
                    ...
                    decoration: _buildInputDecoration('විද්‍යුත් තැපැල් ලිපිනය', 'විද්‍යුත් තැපැල් ලිපිනය ලබා දෙන්න', Icons.email),
                  ...
                  TextField(
                    ...
                    obscureText: true,
                    decoration: _buildInputDecoration('මුරපදය', 'ඔබේ මුරපදය ලබා දෙන්න', Icons.password),
                    style: const TextStyle(color: Colors.white),
                  ),
                ],
              ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _isLoading
                        ...
                        : _buildButton('ප්‍රවේශ වන්න', Styles.successColor, Colors.white, _signIn),
                    const SizedBox(height: 10),
                    Text(
                      'හෝ',
                    ...
                    const SizedBox(height: 10),
                    _buildButton('නව ගිණුමක් සාදන්න', Styles.successColor2, Colors.black87, () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => RegisterUserScreenSinhala()),
                      );
                    }),
                  ],
                ),
