...
origins = [
    "http://localhost:3000",
    "http://localhost:3001",
    "http://localhost:60338",
    "https://202d-111-223-177-154.ngrok-free.app"
]
app.add_middleware(
    CORSMiddleware,
...
def get_growth_records(cattle_id: str):
    ...
    # Get all records from the growth_records subcollection
    growth_ref = animal_ref.collection("growth_records").stream()
    records = [doc.to_dict() for doc in growth_ref]

    return {"growth_records": records}
...
async def create_milk_record(username: str, milk: MilkCollection):
    ...
    return {"message": "Milk collection record added successfully", "milk_record": milk.dict()}
